...
$(document).ready(function () {
    var table = $('#ordenFabricacionTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("corte.getData") }}',  // Ruta para obtener los datos
            type: 'GET',
            dataSrc: 'data',
        },
        columns: [
            { data: 'orden_fabricacion' },
            { data: 'fecha_entrega' },
            { data: 'articulo' },
            {
                data: 'orden_venta_articulo',
        ...
        destroy: true  // Esto permite destruir la instancia anterior de la tabla antes de crear una nueva.
    });
    ...
    const ordenFabricacion = $('#modalBodyContent tr:nth-child(1) td').text().trim().split(': ')[1];
    const fechaEntrega = $('#modalBodyContent tr:nth-child(2) td').text().trim().split(': ')[1];
    const articulo = $('#modalBodyContent tr:nth-child(3) td').text().trim().split(': ')[1];
    const ordenVentaArticulo = $('#modalBodyContent tr:nth-child(4) td').text().trim().split(': ')[1];
    const estado = $('#modalBodyContent tr:nth-child(5) td').text().trim().split(': ')[1];
    ...
    $.ajax({
            ...
            $('#modalDetalleOrden').modal('hide'); // Cerrar el modal
        },
        error: function (xhr, status, error) {
    ...
    document.getElementById('guardarCortes').addEventListener('click', function() {
        const numCortes = document.getElementById('numCortes').value;

        if (!numCortes || numCortes < 0) {
            alert('Por favor, ingresa un número válido de cortes.');
            return;
        }

        fetch('{{ route('guardar.cortes') }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
            },
            body: JSON.stringify({ numCortes: numCortes }),
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                document.getElementById('cortesGuardados').innerText = 'Cortes guardados exitosamente.';
            } else {
                alert('Hubo un problema al guardar los cortes.');
            }
        })
