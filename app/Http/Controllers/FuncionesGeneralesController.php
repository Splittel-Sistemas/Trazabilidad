...
class FuncionesGeneralesController extends Controller
{
    ...
    public function RecibosProduccionParaTraslados($filtro)
    {
        try {
            ...
            $sql = 'SELECT T00."DocNum" AS "RECIBO PRODUCCION", T300."DocNum" AS "ORDEN FABRICACION", T300."OriginNum" AS "ORDEN VENTA", T400."CardCode", T400."CardName",
	                    T100."LineNum", T100."ItemCode", T100."Dscription", SUM(T200."Quantity") AS "Quantity",  SUM(T100."Quantity") AS "Quantity2", T200."BatchNum"
                    FROM ' . $bd . '."OIGN" T00
                    ...
                    WHERE T00."DocNum" >= \'150198\' ' . $filtro . ' AND T100."TranType" IS NOT NULL AND T100."WhsCode" <> \'IMMEX\' AND T300."CardCode" NOT IN (\'C0912\',\'C1000\', \'C0875\')
            ...
            if ($respuestaSAP) {
                while ($row = odbc_fetch_array($respuestaSAP)) {
                    if ($row['Quantity'] > 0) {
                        $datos[$row['RECIBO PRODUCCION']][] = $row;
                    } else {
